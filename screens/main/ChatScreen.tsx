...
export default function ChatScreen({ navigation, route }: ChatScreenProps) {
  ...
  useEffect(() => {
    ...
    const loadInitialProfileImage = async () => {
      try {
        ...
        if (image) {
          // Update URL jika masih pakai IP lama
          const updatedImage = image.replace(/http:\/\/[0-9.]+:3000/, API_CONFIG.BACKEND_URL);
  ...
  // Load profile image on mount and when screen is focused
  useFocusEffect(
    React.useCallback(() => {
...
useEffect(() => {
  ...
  const loadChatHistory = async (preserveScroll = false) => {
    try {
      ...
      } else if (data.success && data.chatHistory) {
      ...
      {showInactiveModal && (
        <View style={styles.deleteModalOverlay}>
          ...
          <View style={styles.deleteModalContainer}>
            ...
            <TouchableOpacity
              style={[styles.deleteModalDeleteButton, { width: '100%' }]}
              onPress={async () => {
            ...
            >
              <Text style={styles.deleteModalDeleteText}>OK</Text>
            </TouchableOpacity>
          </View>
        </View>
      )}
      ...
      >
        ...
        <View style={[styles.header, { backgroundColor: colors.headerBackground, borderBottomColor: colors.border }]}>
          <View style={styles.headerTitleContainer}>
            ...
            <Text style={[styles.headerTitle, { color: colors.text }]}>Asisten Chatbot AI</Text>
          </View>
          <TouchableOpacity
          ...
          >
            {profileImage && !profileImageError ? (
              <Image
                key={`${profileImage}-${imageRetryCount}`}
        ...
        {messages.length === 0 && (
          ...
          >
            ...
            <View style={styles.welcomeTextContainer}>
              <View style={{ overflow: 'hidden' }}>
                <Animated.View
                  style={{
                    ...
                    right: 0,
                    bottom: 0,
                    backgroundColor: colors.background === '#000000' ? 'rgba(255, 255, 255, 0.1)' : 'rgba(255, 255, 255, 0.8)',
                    width: 100,
                    transform: [{
              ...
              <Text style={[styles.welcomeTitle, { color: "#007AFF", fontSize: 20 }]}>
                Asisten Chatbot AI
              </Text>
      ...
      >
        {(showAccountSettings || renderAccountSettings) && (
          <AccountSettingsScreen
            ...
            onClose={async () => {
              setShowAccountSettings(false);
              // Reload profile image after closing settings
              const image = await AsyncStorage.getItem("profileImage");
              if (image) {
                setProfileImage(image);
                setProfileImageError(false);
